...
#pragma endregion
	...
	for (int i = 0; i < entityCount; i++)
	{
		...
		randX = ((rand() % 210) - 100) / 100.0f;
		randY = ((rand() % 210) - 100) / 100.0f;
		entityList[i]->transform->forward =glm::normalize(glm::vec3(randX, randY, 0.0));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Move entites,render them and add them to our quadtree
		for (int i = 0; i < entityCount; i++)
		{
			entityList[i]->transform->position += dt * entityList[i]->transform->forward * 0.2f;
			if (entityList[i]->transform->position.x > 1 || entityList[i]->transform->position.x < -1)
				entityList[i]->transform->position.x *= -1;
			if (entityList[i]->transform->position.y > 1 || entityList[i]->transform->position.y < -1)
				entityList[i]->transform->position.y *= -1;
		...
		//Update array buffer & Draw the quadtree
		glBufferData(GL_ARRAY_BUFFER,
			sizeof(glm::vec2)*linePoints.size(),
			&linePoints[0],
			GL_STATIC_DRAW);
